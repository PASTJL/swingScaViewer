...
class TraiterHotspot5And6 {
  ...
  def retour(tabStr: Array[String]): Double =
    {
      ...
      tabStr(1) match {
        ...
        case "cmsInitialMarkDuration" => TraiterHotspot5And6.structHotSpot.cmsInitialMarkDuration
        case "cmsRemarkDuration" => TraiterHotspot5And6.structHotSpot.cmsRemarkDuration
        case "cmsConcurrentResetDuration" => TraiterHotspot5And6.structHotSpot.cmsConcurrentResetDuration
  ...
  private def remplirStruct(dateCurrent: Long, tabStr: Array[String]) {
    ...
    reg1 = """\[(GC|Full\s+GC).+?(DefNew:|ParNew:|PSYoungGen:|Tenured:|PSOldGen:|ParOldGen:)[^\]]+\]\s+\d+(K|M)""".r
    ext1 = reg1.findFirstIn(tabStr(0))
    if (None != ext1) {
      var ext2 = """\d+(K|M)$""".r.findFirstIn(ext1.get)
      if (ext2.get.contains("K")) {
    ...
    reg1 = """\[(GC|Full\s+GC).+?(DefNew:|ParNew:|PSYoungGen|Tenured:|PSOldGen:|ParOldGen:)[^\]]+\]\s+\d+(K|M)->\d+(K|M)""".r
    ext1 = reg1.findFirstIn(tabStr(0))
    if (None != ext1) {
      var ext2 = """\d+(K|M)$""".r.findFirstIn(ext1.get)
      if (ext2.get.contains("K")) {
 ...
 // remplissage    cmsConcurrentResetDuration
    ...
    reg1 = """CMS-concurrent-reset:\s+[^\]]+""".r
    ext1 = reg1.findFirstIn(tabStr(0))
    if (None != ext1) {
      var ext2 = """\d+(\.|,)\d+\s+secs""".r.findFirstIn(ext1.get)
    
      TraiterHotspot5And6.structHotSpot.cmsConcurrentResetDuration = """\d+(\.|,)\d+""".r.findFirstIn(ext2.get).get.replaceAll(",", ".").toDouble * 1000

    }
     
     // remplissage    cmsInitialMarkDuration
    TraiterHotspot5And6.structHotSpot.cmsInitialMarkDuration = Double.NaN
    reg1 = """CMS-initial-mark:\s+[^\]]+\][^\]]+""".r
    ext1 = reg1.findFirstIn(tabStr(0))
    if (None != ext1) {
      var ext2 = """\d+(\.|,)\d+\s+secs""".r.findFirstIn(ext1.get)
    
      TraiterHotspot5And6.structHotSpot.cmsInitialMarkDuration = """\d+(\.|,)\d+""".r.findFirstIn(ext2.get).get.replaceAll(",", ".").toDouble * 1000
 ...
 // remplissage    cmsRemarkDuration
    ...
    if (None != ext1) {
      ...
      TraiterHotspot5And6.structHotSpot.cmsRemarkDuration = """\d+(\.|,)\d+""".r.findFirstIn(ext2.get).get.replaceAll(",", ".").toDouble * 1000
...
class StructHotspot5And6 {
  ...
  var cmsRemarkDuration = Double.NaN
  var cmsInitialMarkDuration = Double.NaN
  var cmsConcurrentMarkDuration = Double.NaN
  var cmsConcurrentSweep = Double.NaN
  var cmsConcurrentPreclean = Double.NaN
  var cmsConcurrentResetDuration =  Double.NaN
