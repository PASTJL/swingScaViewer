...
object ScaParserMain extends ParsingModes {
  ...
  def apply(conf: java.util.Properties) {
    ...
    props = null
    writer = null
    fileInIncludeReg = null
    fileInExcludeReg = null
    allAverage = null
    patternFin = null
    sdfCsv = null
    stepWithinEnreg = ""
        ...
        for (ij <- 0 until lenVal) {
          ...
          if (tmpUnitValues(ij).equals("ms")
            ...
            title += strVal(ij) + "_Avg" + unites(ij) + fsOut +
              ...
              strVal(ij) + "_Sum_" + (this.stepAgg / 1000) + "s " +
              unites(ij) + fsOut + strVal(ij) +
          ...
          for (ij <- 0 until sizeValues) {
            if (tmpUnitValues(ij).equals("ms")
              ...
              title += tabPiv(i1) + "_" + strVal(ij) + "_Avg" +
                ...
                "_Sum_" + (this.stepAgg / 1000) + "s " +
                unites(ij) + fsOut + tabPiv(i1) + "_" +
              ...
              title += tabPiv(i1) + "_" + strVal(ij) + "_Avg" +
                ...
                strVal(ij) + "_Count_" +
                (this.stepAgg / 1000) + "s (req)" + fsOut +
                tabPiv(i1) + "_" + strVal(ij) + "_Sum_" + (this.stepAgg / 1000) + "s " +
                unites(ij) + fsOut
  ...
  def lancerParsing(conf: java.util.Properties) {
    ...
    for(i <- 1 until nbActors ){ tabBoolSolversStopped(i)=false}
    ...
    while (boolStopGen == false) {
      ...
      try {
        ...
        for (i <- 0 until nbActors) {
         ...
         // println("tabBoolSolversStopped("+i+")="+tabBoolSolversStopped(i))
          // //
        }
        // //
    ...
    val debCsv = System.currentTimeMillis
      var fut: scala.concurrent.Future[akka.actor.Terminated]=    system.terminate()
