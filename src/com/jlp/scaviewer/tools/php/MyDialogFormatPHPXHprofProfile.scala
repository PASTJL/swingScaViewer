...
class MyDialogFormatPHPXHprofProfile(modal:Boolean) extends JDialog with ActionListener {
  ...
  this.setTitle("Parsing XHprof Profile file")
      ...
      if (jb == okB) {
        ...
        class MyRunnable() extends Runnable {
          ...
          override def run {
            ...
            MyDialogFormatPHPXHprofProfile.jdia.setTitle("Please wait while building tree and tables.treatement of file : " + mergedLines.getName())
            MyDialogFormatPHPXHprofProfile.jdia.setVisible(true)
            MyDialogFormatPHPXHprofProfile.jdia.pack()
            MyDialogFormatPHPXHprofProfile.jdia.setLocation(300, 300)
          }
        ...
        if (null == fileLog) {
          ...
          if (fc.getSelectedFile().getName().startsWith("newFormatXHprof_")) {
            mergedLines = new File(fc.getSelectedFile().getParent() + File.separator + fc.getSelectedFile().getName())
          } else {
            mergedLines = new File(fc.getSelectedFile().getParent() + File.separator + "newFormatXHprof_" + fc.getSelectedFile().getName())
          }
          if (!(mergedLines.exists()) || (mergedLines.exists() && new File(fileLog).lastModified() > mergedLines.lastModified())) {
            ...
            XHprofFormatToTree.main(Array(fileLog))
          ...
          class MyRunnableBis extends Runnable {
            ...
            override def run {
              ...
              //PHPJPanelForTreeXHprofProfile.main(args)
              PHPJPanelForTreeXHprofProfile.insideSwingScaViewer(args)
            }
...
object MyDialogFormatPHPXHprofProfile {
