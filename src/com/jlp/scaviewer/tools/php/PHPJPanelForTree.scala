...
//class PHPJPanelForTree(toShow: File) extends JPanel with ActionListener with MouseListener {
class PHPJPanelForTree(toShow: File) extends scala.swing.Panel with ActionListener with MouseListener {  
  ...
  val hmLevels: scala.collection.mutable.HashMap[Int, MyNodeForTree] = new HashMap()

  //HashMap to construct the Table1 Int is the rank of the function
  val hmTable1: scala.collection.mutable.HashMap[Int, MyNodeForTree] = new HashMap()

  //rank, date, nameFunction, fileNameRow, level, nbExec, durationInMillis, selfDurationInMillis, memBefore, memAfter, deltaMem
  val titleTable1: Array[Object] = Array("rank", "nameFunction", "fileNameRow", "level", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")

  // HashMap to construct the Table2 ( concatenation of fonctions)  String  is nameFunction + fileNameRow
  val hmTable2: scala.collection.mutable.HashMap[String, MyNodeForTree] = new HashMap()
  //rank, date, nameFunction, fileNameRow, level, nbExec, durationInMillis, selfDurationInMillis, memBefore, memAfter, deltaMem
  val titleTable2: Array[Object] = Array("nameFunction", "fileNameRow", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")
  ...
  val root = MyNodeForTreeFactory(toShow)
  hmLevels.put(-1, root) // for the root of the Jtree
  
  // JLP To do a fake function with the name of the script.
  var script=MyNodeForTreeFactory(toShow)
  hmLevels.put(0, script)
  // Construction de l'arbre
  //val raf: RandomAccessFile = new RandomAccessFile(toShow, "r")
  // lecture de la premiere ligne => titre
  val reader = initReader(toShow)
  ...
  while (bool) {
    ...
    if (null != line) {
      ...
      val newNode = MyNodeForTreeFactory(line)
    ...
    if (newNode.rank == 0) {
        script.nameFunction =  if (newNode.fileNameRow.contains("/")) {
                 newNode.fileNameRow.substring(newNode.fileNameRow.lastIndexOf("/") + 1).split("_")(0)
              } else  if (newNode.fileNameRow.contains("\\")) {

                newNode.fileNameRow.substring(newNode.fileNameRow.lastIndexOf("\\") + 1).split("_")(0)
              }
              else newNode.fileNameRow.split("_")(0)
                
      }
      val level = newNode.level
//      println("PHPJPanel line ="+line)
//       println("PHPJPanel level ="+level)
...
//  println("taille hmLevels=" + hmLevels.size)
 ...
 // println("hmTable1.size=" + hmTable1.size)
  ...
  // table1 =>"rank", "nameFunction", "fileNameRow", "level", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem"
  ...
  // table2 => "nameFunction", "fileNameRow", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem"
 ...
 table1.getColumnModel().getColumn(idx).setMinWidth(0);
  ...
  val treeSelectionListener: TreeSelectionListener = new TreeSelectionListener() {
    def valueChanged(e: TreeSelectionEvent) {
      val node: MyNodeForTree =
        tree.getLastSelectedPathComponent().asInstanceOf[MyNodeForTree]
      ...
      var rankSelected = 0
      if (node != null) rankSelected = node.rank
  ...
  def getPaths(tree: JTree, parent: TreePath, expanded: Boolean, list: java.util.List[TreePath]) {
    ...
    val node: MyNodeForTree = parent.getLastPathComponent().asInstanceOf[MyNodeForTree];
    if (node.getChildCount() >= 0) {
      val e = node.children()
      while (e.hasMoreElements()) {
        val n: MyNodeForTree = e.nextElement().asInstanceOf[MyNodeForTree]
    ...
    def mouseClicked(e: MouseEvent) {
      ...
      if (e.getSource() == table1) {
        if (e.getButton() > 1) {
          ...
          jTP.remove(idxTab)
          val split = new MyDrillDownXDebugTraceScroll(tree, hmTable1, node)
  ...
  def createConcatNode(tmp1Node: MyNodeForTree, newNode: MyNodeForTree) = {
    val date = ""
    ...
    val memBefore = (tmp1Node.memBefore * tmp1Node.nbExec + newNode.nbExec * newNode.memBefore).toLong / nbExec
    val memAfter = (tmp1Node.memAfter * tmp1Node.nbExec + newNode.nbExec * newNode.memAfter) / nbExec
    val deltaMem = (tmp1Node.deltaMem * tmp1Node.nbExec + newNode.nbExec * newNode.deltaMem) / nbExec

    new MyNodeForTree(rank, date, nameFunction, fileNameRow, level, nbExec, durationInMillis, selfDurationInMillis,
      memBefore, memAfter, deltaMem)
  }
  def returnArrayForTable1(aNode: MyNodeForTree): Array[Object] = {
    //Array("rank", "nameFunction", "fileNameRow", "level", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")
    ...
    returnArray(7) = df.format(aNode.memBefore)
    returnArray(8) = df.format(aNode.memAfter)
    returnArray(9) = df.format(aNode.deltaMem)
    returnArray

  }
  def returnArrayForTable2(aNode: MyNodeForTree): Array[Object] = {
    //Array( "nameFunction", "fileNameRow",  "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")
    ...
    returnArray(5) = df.format(aNode.memBefore)
    returnArray(6) = df.format(aNode.memAfter)
    returnArray(7) = df.format(aNode.deltaMem)
    returnArray
  ...
  val myRenderer: MyDefaultCellXDebugTraceRenderer = new MyDefaultCellXDebugTraceRenderer()
...
// inside of swingScaViewer
  def createAndShowGUIInside(file: File) {
    ...
    val newContentPane = new PHPJPanelForTree(file);
...
case class MyNodeForTree(rank: Int, date: String, var nameFunction: String, fileNameRow: String, level: Int, nbExec: Int, durationInMillis: Double, selfDurationInMillis: Double,
  memBefore: Long, memAfter: Long, deltaMem: Long) extends DefaultMutableTreeNode {
...
val dfs:DecimalFormatSymbols=new DecimalFormatSymbols(Locale.ENGLISH)
  ...
  def toStringMem(): String = {
    rank + ":" + nameFunction + "(" + nbExec + "," + df2.format(memBefore) + "," + df2.format(memAfter) + "," + df2.format(deltaMem) + ")"
  }
  def toStringDuration(): String = {
    rank + ":" + nameFunction + "(" + nbExec + "," + df1.format(durationInMillis) + "," + df1.format(selfDurationInMillis) + ")"
  }

  override def toString(): String = {
    if (PHPJPanelForTree.isDurationView) toStringDuration()
    else
      toStringMem()

  }
  def isEquals(that: MyNodeForTree): Boolean = {

    (this.nameFunction + this.fileNameRow).equals(that.nameFunction + that.fileNameRow)
  }
}
object MyNodeForTreeFactory {
  //date;nameFunction;fileNameRow;level;nbExec;durationInMillis;selfDurationInMillis;memBefore;memAfter;deltaMem);
  def apply(line: String): MyNodeForTree =
    {
      val tab: Array[String] = line.split(";")
      val date = tab(0)
      val rank = tab(1).toInt
      var nameFunction = tab(2)
      val fileNameRow = tab(3)
      val level = tab(4).toInt
      val nbExec = tab(5).toInt
      val durationInMillis = tab(6).toDouble
      val selfDurationInMillis = tab(7).toDouble
      val memBefore = tab(8).toLong
      val memAfter = tab(9).toLong
      val deltaMem = tab(10).toLong
      new MyNodeForTree(rank, date, nameFunction, fileNameRow, level, nbExec, durationInMillis, selfDurationInMillis,
        memBefore, memAfter, deltaMem)

    }
  def apply(file: File): MyNodeForTree =
    {

      val date = ""
      val rank = 0
      var nameFunction = file.getAbsolutePath()
      val fileNameRow = ""
      val level = -1
      val nbExec = 1
