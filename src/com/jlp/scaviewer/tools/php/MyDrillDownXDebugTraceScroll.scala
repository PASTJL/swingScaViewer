...
class MyDrillDownXDebugTraceScroll(tree: JTree, hmTable1: scala.collection.mutable.HashMap[Int, MyNodeForTree],
  node: MyNodeForTree) extends JScrollPane with MouseListener {
  ...
  // Table 1 de PHPJPanel
  // "rank", "nameFunction", "fileNameRow", "level", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem"

  val titleTableParent: Array[Object] = Array("rank", "fileNameRow", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")
  val titleTableFunction: Array[Object] = Array("rank", "fileNameRow", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")

  val titleTable1: Array[Object] = Array("rank", "fileNameRow", "nameFunction", "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem")
  ...
  // table1 =>"rank","nameFunction","nbExec","durationInMillis","selfDurationInMillis","cpuDurationInMillis","cpuDurationInMillis","memoryUsed","peakMemory"
  sorter1.setComparator(0, new ComparatorStringAsLong())
  sorter1.setComparator(2, new ComparatorStringAsLong())
  sorter1.setComparator(3, new ComparatorStringAsDouble())
  ...
  private def fillDataOneLine(idNode: Int, aTable: JTable) {
    ...
    // val titleTableParent:"rank","fileNameRow",  "nbExec", "durationInMillis", "selfDurationInMillis", "memBefore", "memAfter", "deltaMem"
    val locDatas: Array[Object] = Array(node.rank.toString, node.fileNameRow, node.nbExec.toString, df1.format(node.durationInMillis), df1.format(node.selfDurationInMillis),
      df.format(node.memBefore), df.format(node.memAfter), df.format(node.deltaMem))
    //    val locDatas: Array[Object] = Array(node.rank.toString,node.fileNameRow,node.nameFunction, node.nbExec.toString, node.durationInMillis.toString, node.selfDurationInMillis.toString,
    //      node.memBefore.toString, node.memAfter.toString, node.deltaMem.toString)
  ...
  def returnArrayForTableChildren(aNode: MyNodeForTree): Array[Object] = {
    ...
    returnArray(6) = df.format(aNode.memBefore)
    returnArray(7) = df.format(aNode.memAfter)
    returnArray(8) = df.format(aNode.deltaMem)

    returnArray

  }

  def fillAgain(newNode: MyNodeForTree) {
