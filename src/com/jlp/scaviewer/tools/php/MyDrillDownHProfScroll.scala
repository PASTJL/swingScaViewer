...
class MyDrillDownHProfScroll(tree: JTree, hmTable1: scala.collection.mutable.HashMap[Int, MyNodeForTreeXHprofProfile],
  node: MyNodeForTreeXHprofProfile) extends JScrollPane with MouseListener {
  ...
  val titleTableFunction: Array[Object] = Array("rank", "nbExec", "durationInMillis", "selfDurationInMillis", "cpuDurationInMillis", "selfCpuDurationInMillis", "memoryUsed", "peakMemory")

  val titleTable1: Array[Object] = Array("rank", "nameFunction", "nbExec", "durationInMillis", "selfDurationInMillis", "cpuDurationInMillis", "selfCpuDurationInMillis", "memoryUsed", "peakMemory")
  ...
  private def fillDataOneLine(idNode: Int, aTable: JTable) {
    ...
    // val titleTableParent: Array[Object] = Array("rank","nbExec", "durationInMillis", "selfDurationInMillis", "cpuDurationInMillis", "selfCpuDurationInMillis", "memoryUsed", "peakMemory")
    val locDatas: Array[Object] = Array(node.rank.toString, node.nbExec.toString, df1.format(node.durationInMillis), df1.format(node.selfDurationInMillis),
      df1.format(node.cpuDurationInMillis), df1.format(node.selfCpuDurationInMillis), df.format(node.memoryUsed), df.format(node.peakMemory))
  ...
  def returnArrayForTableChildren(aNode: MyNodeForTreeXHprofProfile): Array[Object] = {
    //// table1 =>"rank","nameFunction","nbExec","durationInMillis","selfDurationInMillis","cpuDurationInMillis","cpuDurationInMillis","memoryUsed","peakMemory"
    ...
    returnArray(5) = df1.format(aNode.cpuDurationInMillis)
    returnArray(6) = df1.format(aNode.selfCpuDurationInMillis)
    returnArray(7) = df.format(aNode.memoryUsed)
    returnArray(8) = df.format(aNode.peakMemory)
    returnArray

  }

  def fillAgain(newNode: MyNodeForTreeXHprofProfile) {
